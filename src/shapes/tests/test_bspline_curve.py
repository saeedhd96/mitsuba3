...
def test03_parameters_changed(variant_cuda_ad_rgb):
    ...
    new_control_points = mi.Point3f(new_control_points)

    new_radiuses = np.array([
        1,
        2,
        3,
        4,
    ])
    new_radiuses = mi.Float(new_radiuses)
    ...
    params['radiuses'] = new_radiuses
    params.update()

    params = mi.traverse(s)
    assert dr.allclose(dr.ravel(new_control_points), params['control_points'])
    assert dr.allclose(new_radiuses, params['radiuses'])
...
def test07_differentiable_surface_interaction_ray_forward_follow_shape(variant_llvm_ad_rgb):
    ...
    params['curve.control_points'] = dr.ravel(dr.unravel(
        mi.Point3f, params['curve.control_points']) + mi.Vector3f([theta, 0, 0])
    )
    params.update()
